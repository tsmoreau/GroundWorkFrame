...
function Field({
  ...
  return (
    <div>
      ...
      {hint && <p className="text-xs text-[var(--color-stone)] mb-1.5">{hint}</p>}
      {children}
    </div>
  );
}
...
function TextInput({
  ...
  return (
    <input
      ...
      className="w-full text-sm border border-[var(--color-border)] rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[var(--color-espresso)]/20 bg-white"
    />
  );
}
...
function Toggle({
  ...
  return (
    <label className="flex items-center gap-2 cursor-pointer">
      <div
        className={`relative w-9 h-5 rounded-full transition-colors ${checked ? "bg-[var(--color-gold)]" : "bg-[var(--color-sand)]"}`}
  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="mb-6">
        ...
        <p className="text-sm text-[var(--color-stone)] mt-1">Business configuration and payment methods</p>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-[var(--color-border)] p-6">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              {form.stripeEnabled && (
                <p className="text-xs text-[var(--color-stone)] mt-2 pl-11">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={handleSave}
            className="px-6 py-2.5 rounded-lg text-sm font-medium text-[var(--color-charcoal)]"
            style={{ backgroundColor: "var(--color-gold)" }}
          >
            Save Settings
          </button>
          {saved && (
            <span className="text-sm text-[var(--color-paid)] font-medium">Saved</span>
          )}
        </div>
      </div>
    </div>
