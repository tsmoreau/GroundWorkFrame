...
function Field({
  ...
  return (
    <div>
      ...
      {hint && <p className="text-xs text-[#a09890] mb-1.5">{hint}</p>}
      {children}
    </div>
  );
}
...
function TextInput({
  ...
  return (
    <input
      ...
      className="w-full text-sm border border-[#e0dbd0] rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#1c3829]/20 bg-white"
    />
  );
}
...
function Toggle({
  ...
  return (
    <label className="flex items-center gap-2 cursor-pointer">
      <div
        className={`relative w-9 h-5 rounded-full transition-colors ${checked ? "bg-[#1c3829]" : "bg-[#d0cbc4]"}`}
...
export default function SettingsPage() {
  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[#1a1a18]">Settings</h1>
        <p className="text-sm text-[#5c5c54] mt-1">Business configuration and payment methods</p>
      </div>

      <div className="space-y-6">
        {/* Business Info */}
        <div className="bg-white rounded-xl border border-[#e0dbd0] p-6">
          <h2 className="font-semibold text-[#1a1a18] mb-5">Business Information</h2>
        ...
        {/* Invoice Settings */}
        <div className="bg-white rounded-xl border border-[#e0dbd0] p-6">
          <h2 className="font-semibold text-[#1a1a18] mb-5">Invoice Settings</h2>
        ...
        {/* Payment Methods */}
        <div className="bg-white rounded-xl border border-[#e0dbd0] p-6">
          <h2 className="font-semibold text-[#1a1a18] mb-5">Payment Methods</h2>
          <div className="space-y-6">
            {/* Stripe */}
            <div>
              <Toggle checked={form.stripeEnabled} onChange={(v) => set("stripeEnabled", v)} label="Stripe (ACH + Card)" />
              {form.stripeEnabled && (
                <p className="text-xs text-[#5c5c54] mt-2 pl-11">
        ...
        <div className="flex items-center gap-3">
          ...
          {saved && (
            <span className="text-sm text-[#2d6a4f] font-medium">Saved</span>
          )}
        </div>
      </div>
    </div>
