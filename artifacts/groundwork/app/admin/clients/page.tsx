  ...
  return (
    <div className="p-8 max-w-6xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#1a1a18]">Clients</h1>
          <p className="text-sm text-[#5c5c54] mt-1">{clients.length} total</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-[#e0dbd0] overflow-hidden">
              ...
              return (
                <tr key={client.id} className="hover:bg-[#faf8f5] transition-colors">
                  <td className="px-4 py-3">
                    <div>
                      <p className="font-medium text-sm text-[#1a1a18]">{client.name}</p>
                      <p className="text-xs text-[#5c5c54] mt-0.5">{client.email}</p>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      <PawPrint className="w-3.5 h-3.5 text-[#a09890]" />
                      <span className="text-xs text-[#5c5c54]">
                        {client.pets.map((p) => p.name || p.breed || p.type).join(", ")}
                      </span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-xs text-[#5c5c54] max-w-[180px] truncate">
                    {client.address}
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-semibold text-[#1a1a18]">{clientJobs.length}</span>
                      {activeJobs.length > 0 && (
        ...
        {filtered.length === 0 && (
          <div className="text-center py-12 text-[#5c5c54] text-sm">No clients found.</div>
        )}
      </div>
    </div>
  );
