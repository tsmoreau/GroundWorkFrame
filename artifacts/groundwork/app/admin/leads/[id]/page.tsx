...
export default function LeadDetailPage({
  ...
  if (!lead) {
    return (
      <div className="p-8">
        <p className="text-[#5c5c54]">Lead not found.</p>
        <Link href="/admin/leads" className="text-sm text-[#1c3829] hover:underline mt-2 inline-block">
          Back to leads
        </Link>
      </div>
    );
  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="flex items-center gap-3 mb-6">
        <Link
          href="/admin/leads"
          className="flex items-center gap-1 text-sm text-[#5c5c54] hover:text-[#1a1a18] transition-colors"
        >
          <ChevronLeft className="w-4 h-4" />
          Leads
        </Link>
        <span className="text-[#d0cbc4]">/</span>
        <span className="text-sm text-[#1a1a18] font-medium">{lead.name}</span>
      </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 space-y-5">
          ...
          <div className="bg-white rounded-xl border border-[#e0dbd0] p-6">
            <div className="flex items-start justify-between">
              <div>
                <h1 className="text-xl font-bold text-[#1a1a18]">{lead.name}</h1>
                <p className="text-xs text-[#5c5c54] mt-1">
              ...
              <select
                ...
                className="text-sm border border-[#e0dbd0] rounded-lg px-3 py-1.5 focus:outline-none"
                disabled={lead.status === "converted"}
                style={{ color: STATUS_COLORS[lead.status] }}
              >
                {STATUS_OPTIONS.map((s) => (
                  <option key={s} value={s} className="text-[#1a1a18] capitalize">
            ...
            <div className="mt-4 space-y-2">
              <div className="flex items-center gap-2 text-sm text-[#5c5c54]">
                ...
                <a href={`mailto:${lead.email}`} className="hover:text-[#1c3829]">
                  {lead.email}
                </a>
              </div>
              {lead.phone && (
            ...
            <div className="mt-4 pt-4 border-t border-[#f0ece4]">
              <p className="text-xs font-semibold text-[#5c5c54] uppercase tracking-wider mb-2 flex items-center gap-1.5">
                <MessageSquare className="w-3.5 h-3.5" />
                Message
              </p>
              <p className="text-sm text-[#1a1a18] leading-relaxed">{lead.message}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-[#e0dbd0] p-6">
            ...
            {lead.notes.length === 0 && (
              <p className="text-sm text-[#5c5c54] mb-4">No notes yet.</p>
            )}
            <div className="space-y-3 mb-4">
              {lead.notes.map((note, i) => (
                <div key={i} className="relative pl-4 border-l-2 border-[#e0dbd0]">
                  <p className="text-sm text-[#1a1a18] leading-relaxed">{note.text}</p>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-xl border border-[#e0dbd0] p-5">
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-sm">
                <span className="text-[#5c5c54]">Type</span>
                <span className="font-medium capitalize">{lead.petInfo.type}</span>
              </div>
              {lead.petInfo.breed && (
                <div className="flex justify-between text-sm">
                  <span className="text-[#5c5c54]">Breed</span>
                  <span className="font-medium">{lead.petInfo.breed}</span>
                </div>
              )}
              {lead.petInfo.count && (
                <div className="flex justify-between text-sm">
                  <span className="text-[#5c5c54]">Count</span>
          ...
          {lead.address && (
            <div className="bg-white rounded-xl border border-[#e0dbd0] p-5">
              <h3 className="font-semibold text-sm text-[#1a1a18] mb-3 flex items-center gap-2">
                <MapPin className="w-4 h-4 text-[#5c5c54]" />
                Location
              </h3>
              <p className="text-xs text-[#5c5c54] mb-3">{lead.address}</p>
              <a
                href={`https://maps.google.com/?q=${encodeURIComponent(lead.address)}`}
                target="_blank"
                rel="noopener noreferrer"
          ...
          {/* Actions */}
          <div className="bg-white rounded-xl border border-[#e0dbd0] p-5">
            <h3 className="font-semibold text-sm text-[#1a1a18] mb-3">Actions</h3>
            {lead.status === "converted" && lead.convertedTo ? (
              <Link
                href={`/admin/clients/${lead.convertedTo}`}
                className="flex items-center justify-between w-full px-4 py-2.5 rounded-lg text-sm font-medium border border-[#e0dbd0] text-[#1c3829] hover:bg-[#f8f6f2]"
              >
                View Client Record
